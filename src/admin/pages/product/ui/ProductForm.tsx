...
export const ProductForm = ({ title, subTitle, product, isMutating, onSubmit }: Props) => {
    ...
    const addTag = () => {
        if (inputRef.current?.value.trim() !== '' && inputRef.current?.value && !getValues('tags').includes(inputRef.current.value)) {
            const tagsSet = new Set(
                getValues('tags').map(tag => tag.toUpperCase())
            );
            tagsSet.add(inputRef.current.value.trim().toUpperCase());
            setValue('tags', Array.from(tagsSet));
        }
    }

    const removeTag = (tagToRemove: string) => {
        const newTags = getValues('tags').filter(tag => tag !== tagToRemove.toUpperCase())
    ...
    const removeSize = (sizeToRemove: Size) => {
        const newSizes = getValues('sizes').filter(size => size !== sizeToRemove);
