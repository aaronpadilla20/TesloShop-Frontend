...
export const ProductsGrid = ({ products }: Props) => {
    ...
    useLayoutEffect(() => {
        const updateViewModeByDevice = () => {
            const mode = window.innerWidth <= 768 ? 'list' : 'grid'; // móvil vs desktop
        ...
        // Detectar al montar
        updateViewModeByDevice();

        // Detectar cambios de tamaño
        window.addEventListener('resize', updateViewModeByDevice);
        return () => window.removeEventListener('resize', updateViewModeByDevice);
    }, [searchParams]); // ⚠️ searchParams en deps porque lo usamos en el effect
